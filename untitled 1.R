...
for (h in 1:1){
  station <- data.frame(matrix(ncol = 6, nrow= 0))
  cnames_station <- c("nve_nb, long", "lat", "FPY", "KS", "AD")
  ...
  FGP <- data.frame(felt_data$STASJON_NR, felt_data$QD_fgp, felt_data$QD_median, felt_data$latitude, felt_data$longitude)
  colnames(FGP) <- c("Station","FGP", "Average yearly flood", "lat", "long")
...
for (s in 1:length(stations30$Stations)){
  ...
  if(length(pot_river$FGP) == 0){
    ...
    #ams_river <- ams_data[ams_data$station == "89.1", ]
    #pot_river <- pot_data[pot_data$station == "2.11", ]
    station_values <- data.frame(stations30$Stations[s],stations30$long[s],stations30$lat[s])
    colnames(station_values) <- c("Stations", "long", "lat")
    ...
    for(y in 1:1){
      ...
      newrow_station <-  data.frame("nve_nb" = station_values$Stations, "long" = station_values$long, "lat" = station_values$lat,
                                    "FPY" = testplotframe$`Floods per year`[1], "KS" = KSmin$KS, "AD" = ADmin$AD) 
      station <- rbind(newrow_station, station)
      write.table (station, file = "station values30.txt", sep = "\ ")
      ...
      write.table (gev_gofvalues, file = "gev values30.txt", sep = "\ ")
      ...
      write.table (gp_gofvalues, file = "gp values30.txt", sep = "\ ")
      ...
      write.table (gumbel_gofvalues, file = "gumbel values30.txt", sep = "\ ")
      ...
      write.table (exp_gofvalues, file = "exp values30.txt", sep = "\ ")
      ...
      write.table (return_l, file = "Return level averages30.txt", sep = "\ ")
      ...
      write.table (CoefficientVariance, file = "Coefficient of variation stability test30.txt", sep = "\ ")
